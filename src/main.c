...
void print_usage_and_exit(const char* prog) {
	printf("usage: %s <matrix-in-file> [-p|--png pattern_name] [-t|--threshold eps] [-o|--original_rcm] [-g|--graph_threshold eps]\n", prog);
	exit(2);
}
...
int main(int argc, char** argv) {
	...
	// original matrix
	if ( config.portrait_file != NULL ) matrix_portrait_pattern(&matr, config.portrait_file, "_asrc", config.graph_threshold);

	rcm(&matr, config.threshold);
	if ( config.portrait_file != NULL ) matrix_portrait_pattern(&matr, config.portrait_file, (config.threshold==0)?"_orcm":"_zrcm", config.graph_threshold);
